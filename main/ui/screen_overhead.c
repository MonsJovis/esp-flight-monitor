...
/* True from create() to the first update: the card knows nothing yet. */
static bool      s_unfilled;
...
/* The card before its first update (D85, D86). Without this it was the
 * compass tape pointing north at nothing and no way back, for up to the two
 * seconds until ui_task's next tick. open_detail() now wakes ui_task, so this
 * is on the glass for a moment at most — and for that moment the card is
 * empty but for "Zurück". D85 put a whole-card skeleton here and held it for
 * 600 ms on every open; on the panel that was too much, so it is gone. */
static void start_unfilled(void)
{
    s_unfilled = true;
...
void screen_overhead_update(const view_model_t *vm)
{
    ...
    /* Every label below sets its own visibility, so the first update fills
     * the card whatever start_unfilled() hid. */
    s_unfilled = false;
...
void screen_overhead_set_back_cb(void (*cb)(void))
{
    ...
    /* A card not yet filled can already be left (D85). */
    if (s_unfilled && cb != NULL) {
