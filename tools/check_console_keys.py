...
checks, so `on_cmd()` in main/main.c has a key for each one. THREE places
describe that console — the comment block at the top of main.c, the "ready:"
line the firmware prints at boot, and the list in AGENTS.md §3 — and none of
them is consulted by the code, so all three drift the moment someone adds a
key and forgets. AGENTS.md's is the one that matters most: it is the first
thing anyone new reads, and the only one they see before concluding that a
screen cannot be reached from the build host.
...
header either. Extending it to AGENTS.md found `x` had never been there at
all — the touch report, which is the only way to see what the touch layer
...
def documented(text):
    ...
    Deliberately generous about HOW a key is written, because the three blobs are
    ...
    """
    # Markdown backticks are markup, not content: AGENTS.md writes its range
    # as "`1`-`5`", and a parser that cannot see through that reports two keys
    # as undocumented when they are sitting right there. A gate that cries
    # wolf gets worked around instead of fixed.
    text = text.replace("`", "")

    found = set()
    # Hyphen, en dash or em dash: the three prose lists use all of them
    # ("1-5", "`1`–`5`"), and a range this does not recognise reads as two
    # undocumented keys — a false alarm, which is how a gate earns a
    # reputation for being worked around instead of fixed.
    for lo, hi in re.findall(
            r"(?<![A-Za-z0-9])([0-9A-Za-z])\s*[-–—]\s*([0-9A-Za-z])(?![A-Za-z0-9])",
            text):
...
def agents_section():
    """AGENTS.md §3's console list, between its two stable headings."""
    doc = (ROOT / "AGENTS.md").read_text(encoding="utf-8")
    start, end = "### The loop you actually work in", "Do **not** use Arduino"
    if start not in doc or end not in doc:
        sys.exit("check_console_keys: AGENTS.md §3 no longer has the headings "
                 "this reads between (%r ... %r). Point it at the new ones in "
                 "the same commit that moves them." % (start, end))
...
def blobs():
    ...
    if m is None:
        sys.exit("check_console_keys: the boot 'ready:' line is gone from "
                 "main/main.c. It is one of the three things this checks; "
...
def main(argv):
    ...
    if problems:
        ...
        for p in problems:
            print("  " + p)
        print("\nEvery key on_cmd() answers to has to appear in all three, because "
              "they are the only places anyone will look. A key that works "
              "and is written down nowhere puts the screen behind it back to "
              "being one nobody checks (D4, D41).")
        return 1

    print("console keys: all %d are named in the header, the ready line and "
          "AGENTS.md §3" % len(keys))
    return 0
