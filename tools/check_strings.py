...
_NON_UI_CALLS = frozenset({
    ...
    "ESP_ERROR_CHECK",
    # A static assertion's message reaches a compiler diagnostic and nothing
    # else — it cannot be drawn even in principle, because the build that
    # would have drawn it does not exist. Both spellings: C11 has the
    # underscore form, <assert.h> defines the other.
...
# Files written by a generator, whose every literal is machine data by
# construction. They are NOT exempt on their name alone: each one has to still
# carry the marker below, checked at scan time. A generated file that someone
# has started hand-editing loses the marker in the same edit — and with it the
# exemption — which is the only version of "trust the generator" that stays
# true. Fix the generator, then re-run it; do not edit these.
#
# main/net/tz_table.h is 114 IANA zone names and 48 POSIX TZ strings from real
# tzdata (tools/build_tz_table.py). "CET-1CEST,M3.5.0,M10.5.0/3" is not
# something a native speaker can usefully be asked to read aloud, and it is
# not something anyone should be able to edit by hand either.
GENERATED_FILES = {
    "main/net/tz_table.h": "GENERATED by tools/build_tz_table.py",
}
...
def enforced_files():
    ...
    for path in sorted(set((ROOT / "main").rglob("*.c"))
        ...
        if any(rel.startswith(d) for d in EXEMPT_DIRS):
            continue
        marker = GENERATED_FILES.get(rel)
        if marker is not None:
            # The exemption has to keep proving itself: no marker, no pass.
            if marker in path.read_text(encoding="utf-8", errors="replace"):
                continue
            sys.exit(
                "user-facing strings: %s is listed in GENERATED_FILES but no "
                "longer says '%s'.\n\nEither it was hand-edited (fix "
                "tools/build_tz_table.py and re-run it instead), or the "
                "generator stopped writing its own banner. Until one of those "
                "is true the exemption does not apply, because an exemption "
                "that cannot check itself is how a gate quietly stops "
                "checking." % (rel, marker)
            )
        files.append(path)
